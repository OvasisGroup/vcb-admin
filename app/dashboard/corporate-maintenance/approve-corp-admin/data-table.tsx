...
export function DataTable<TData, TValue>({
  ...
  const table = useReactTable({
    ...
    meta: {
      onApprove: async (adminDraftId: number) => {
        try {
          await onApprove(adminDraftId);
        } catch (error) {
          console.error("Error approving corporate admin:", error);
        }
      },
      onReject: async (adminDraftId: number) => {
        try {
          await onReject(adminDraftId);
        } catch (error) {
          console.error("Error rejecting corporate admin:", error);
        }
      },
    },
  });
    ...
    <div>
      <div className="mb-4 flex justify-between items-center">
        <Input
          ...
          value={(table.getColumn("adminDraftId")?.getFilterValue() as string) ?? ""}
          onChange={(event) =>
            table.getColumn("adminDraftId")?.setFilterValue(event.target.value)
          }
          className="max-w-sm"
        />
